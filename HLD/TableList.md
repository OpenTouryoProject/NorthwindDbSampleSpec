...
基本：[共通仕様](./Common.md) / [機能一覧](./FeatureList.md) / **テーブル一覧** / [画面一覧](./UI_List.md) / [画面遷移](./UI_FlowList.md)<br>
...
**外部キー制約は DB に定義しない**（[DB に持たせない制約](../LLD/TableSchema.md#db-に持たせない制約)）。
下表の判定は**アプリケーションが削除の前に実施**し、該当する場合は `ERR-FK`（[共通仕様 7 節](./Common.md#7-共通エラーハンドリング)）とする。
DB は削除を拒否しないため、この確認を省略すると不整合なデータが残る。
...
`SalesTargets` も社員の削除時にあわせて削除する。
**CASCADE は使わない**ため、これらの削除はアプリケーションが同一トランザクション内で順に実行する。
